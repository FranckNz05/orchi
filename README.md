...
- revenu = **aucun abonnement**, mais 5 % du montant de chaque transaction
  réussie moins la commission de l'agrégateur, perçus **hors flux** — c'est bien
  une marge sur le GMV, encaissée sans jamais détenir les fonds ;
...
**Cette décision a été arbitrée explicitement**, contre l'alternative collecteur :
prélever 5 % sur chaque transaction *suggère* d'être dans le flux des fonds,
mais ne l'exige pas. Le montant et le chemin sont deux questions distinctes, et
`PLATFORM_FEE_COLLECTION` traite la seconde — voir [Tarification](#tarification).

Le modèle collecteur (encaissement sur comptes propres + reversement avec marge)
reste la cible **par pays**, une fois le volume acquis et les accords
sub-merchant signés. Il achète la certitude d'être payé et un routage affranchi
de l'onboarding du marchand, au prix d'un agrément (agent ou EME), de comptes
cantonnés et d'un dispositif AML/CFT. Le schéma de données est prévu pour cette
bascule sans migration destructrice ; le reste ne l'est pas.
...
#### Comment cette part est perçue

Le taux ne dit pas par quel chemin l'argent arrive. C'est le rôle de
`PLATFORM_FEE_COLLECTION`, qui ne change **jamais** le calcul — `pricing.ts`
produit le même montant dans tous les cas — mais uniquement les écritures du
grand livre :

| Mode | Mécanisme | Prérequis |
|---|---|---|
| `invoice` **(défaut)** | La part est constatée en créance sur `merchant:{id}:billing`, puis facturée. | Aucun |
| `split` | L'agrégateur reverse directement la part Orchi, réellement retenue sur le flux. | Accord sub-merchant |
| `on_top` | *Non implémenté — le serveur refuse de démarrer.* | Majorer le montant demandé au client final |

`invoice` est le défaut parce que c'est **le seul mode compatible avec le
modèle A** : ne détenant pas les fonds, Orchi ne peut rien retenir dessus. Le
marchand encaisse donc l'intégralité de ce que verse son agrégateur, et règle
Orchi ensuite — ce qui joue en sa faveur en trésorerie.

`split` reste la cible : il supprime le risque d'impayé **sans** faire détenir
un centime à Orchi. Il s'active agrégateur par agrégateur, à mesure que les
accords se signent.

### Ledger

Partie double. En mode `invoice`, un encaissement de 15 000 XOF avec un
agrégateur à 2 % produit **deux journaux**, écrits dans la même transaction :

```
payin.succeeded   DEBIT  provider:cinetpay:clearing  15000
                  CREDIT merchant:mch_x:receivable   14700
                  CREDIT provider:cinetpay:fees        300

fee.accrued       DEBIT  merchant:mch_x:billing        450
                  CREDIT orchi:revenue                 450
```

Les séparer n'est pas cosmétique : le marchand reçoit **l'intégralité** de ce
que verse l'agrégateur (15 000 − 300), et la commission Orchi est une créance à
facturer, pas un prélèvement sur le flux. Le solde de `merchant:{id}:billing`
est exactement ce qu'une facture devra réclamer.

En mode `split`, la part Orchi étant réellement retenue par l'agrégateur, un
seul journal la constate : `merchant:mch_x:receivable` est crédité de 14 250 et
`orchi:revenue` de 450, sans `fee.accrued`.
